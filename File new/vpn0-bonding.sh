...
if [ "$newstatus" = "tuntap_up" ]; then
    ...
    /sbin/ip route add 10.10.0.0/24 dev $tuntap_intf
    /sbin/iptables -t nat -A POSTROUTING -o enp0s5 -s 10.10.0.0/24 -j MASQUERADE
elif [ "$newstatus" = "tuntap_down" ]; then
    /sbin/iptables -t nat -D POSTROUTING -o enp0s5 -s 10.10.0.0/24 -j MASQUERADE
fi
